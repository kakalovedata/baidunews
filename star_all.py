...
def urls_get(keyword,num=10):
    url_frist='https://www.baidu.com/s?ie=utf-8&cl=2&medium=0&rtt=4&bsst=1&rsv_dl=news_t_sk&tn=news&word={}'
    urls=[]
...
def get_infomation_url(url):
    ...
    for html_key_div in html_key_divs:
        ...
        'detail':html_key_div.select('div')[-1].get_text().replace('\n','').replace('\t','').split(' ,')[-1]
        }